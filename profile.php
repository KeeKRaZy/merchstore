...
if (isset($_POST['button'])) {
    ...
    $password = md5($password."b7n823mydx18762c");
    $emailInputed = $_POST['email'];

    $query = mysqli_query($start, "SELECT * FROM `users` WHERE `email`='{$usersEmail}' AND `password`='{$password}';");

    if (mysqli_num_rows($query) == 1) {
        $query = mysqli_query($start, "SELECT * FROM `users` WHERE `email`='{$emailInputed}';");

        if (mysqli_num_rows($query) == 0 || $emailInputed == $usersEmail) {
            $nameInputed = $_POST['name'];
            $surnameInputed = $_POST['surname'];
            $adressInputed = $_POST['adress'];
        
            $query = mysqli_query($start, "UPDATE `users` SET email = '$emailInputed', `name` = '$nameInputed', surname = '$surnameInputed', adress = '$adressInputed' WHERE email LIKE '$usersEmail';");
            
            setcookie('user', $usersEmail, time() - 3600, "/");
            setcookie('user', $emailInputed, time() + 3600, "/");
        
            $usersEmail = $emailInputed;
        }
    }

}

if (isset($_POST['buttonForPass'])) {
    
    $currentPassword = $_POST['currentpass'];
    $currentPassword = md5($currentPassword."b7n823mydx18762c");

    $query = mysqli_query($start, "SELECT * FROM `users` WHERE `email`='{$usersEmail}' AND `password`='{$currentPassword}';");
    
    if (mysqli_num_rows($query) == 1) {

        $newPassword = $_POST['newpass'];
        $newPassword = md5($newPassword."b7n823mydx18762c");

        $query = mysqli_query($start, "UPDATE `users` SET password = '$newPassword' WHERE email LIKE '$usersEmail';");
        
    }

}

$query = mysqli_query($start, "SELECT * FROM users WHERE email LIKE '$usersEmail'");
    ...
    <header>
        <nav>
            <ul>
                ...
                <li><a href="index.php"><h1>YOUR PROFILE</h1></a></li>
                <li>
    ...
    <main>
        <div class="usersProfile">
            ...
            if(isset($_COOKIE['admin'])){
                echo'<i id="profileLogo" class="fa-solid fa-user-secret" aria-hidden="true"></i>';
            } else{
                echo'<i id="profileLogo" class="fa fa-user" aria-hidden="true"></i>';
            };
            ?>
            <div id="usersData">
                <?php
                    echo"<p>Name: $name $surname</p>
                        <p>E-mail: $usersEmail</p>";
                        if(isset($_COOKIE['admin'])){
                            echo"<p>Role: admin</p>";
                        } elseif (empty($adress)){
                            echo"<p>Adress: <span style='color: grey;'>not specified</span></p>";
                        } else{
        ...
        <div class="changeYourData">
            <p>CHANGE YOUR DATA</p>
            <form method="POST" onsubmit="return validationCheck()">
                <div> 
                    <input id="userNameInsideInput" type="text" name="name" minlength="2" maxlength="15" required placeholder="Enter new name">
                    <input type="text" id="secondShortInput" name="surname" minlength="2" maxlength="15" required placeholder="Enter new surname">
                </div>
                <input id="emailInsideInput" class="longInput" type="email" name="email" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+.[a-z]{2,4}"  minlength="5" maxlength="49" required placeholder="Enter new e-mail">
                <?php
                if(!isset($_COOKIE['admin'])){
                    echo'<input id="insideInputAdress" class="longInput" type="text" name="adress" maxlength="29" placeholder="Enter your adress">';
                };
                ?> 
                <li>Confirm with current password</li>
                <input id="passwordForDataChange" type="password" minlength="8" maxlength="20" name="confirm" required placeholder="Your password">
                <input id="updateButton" type="submit" name="button" value="Update">
            </form>
        </div>
        <div class="changeYourData changePassword">
            <p>CHANGE PASSWORD</p>
            <form method="POST" onsubmit="return validationCheck()">
                <div> 
                    <input type="password" name="currentpass" minlength="8" maxlength="20" required placeholder="Enter current password">
                    <input type="password" id="secondShortInput" minlength="8" maxlength="20" name="newpass" required placeholder="Enter new password">
                </div>
                <input id="updateButton" type="submit" name="buttonForPass" value="Update">
            </form>
        </div>
        <?php 
        if(!isset($_COOKIE['admin'])){
            echo'<div class="ordersHistory">
                <p>ORDERS HISTORY</p>
                <div class="ordersHistoryHeader">
                    <li class="historyNr">Nr.</li>
                    <li class="historyDate">Date</li>
                    <li class="historyStatus">Status</li>
                </div>';


            $query = mysqli_query($start, "SELECT * FROM `orders` WHERE `email`='{$usersEmail}' ORDER BY `date` DESC;");
            foreach($query as $row) {
...
<?php
echo"<script>
        document.getElementById('userNameInsideInput').value = '$name';
        document.getElementById('secondShortInput').value = '$surname';
        document.getElementById('emailInsideInput').value = '$usersEmail';
    </script>";
if(!isset($_COOKIE['admin'])){
    echo"<script>
        document.getElementById('insideInputAdress').value = '$adress';
    </script>";
}
...
if (isset($_POST['buttonForPass'])) {
    ...
    if (!isset($newPassword)){
        ...
        echo"<script>
                swal(`Success!`, `Password was updated!`, `success`);
            </script>";
